...
void fullSimShape(string mode) {
  ...
  for (unsigned int i = 0; i != vars.size(); ++i) {
    ...
    // Constrain hhnoise in data at high pT
    if (sv2=="hhnoise") {
      int n = hv->GetNbinsX();
      int n2 = hv->GetXaxis()->FindBin(1700.);
      hv->SetBinError(n,0.01);
      hv->SetBinError(n2,0.01);
      // RunGfix versions
      if (obs=="Rjet") hv->SetBinContent(n,-0.05);
      if (obs=="chf")  hv->SetBinContent(n,+0.015);
      if (obs=="nhf")  hv->SetBinContent(n,-0.025);
      if (obs=="gammaf")  hv->SetBinContent(n,+0.010);
      //
      if (obs=="Rjet") hv->SetBinContent(n2,-0.10);
      if (obs=="chf")  hv->SetBinContent(n2,+0.03);
      if (obs=="nhf")  hv->SetBinContent(n2,-0.05);
      if (obs=="gammaf")  hv->SetBinContent(n2,+0.02);
      // RunGC versions
      //if (obs=="Rjet") hv->SetBinContent(n,-0.10);
      //if (obs=="chf")  hv->SetBinContent(n,+0.05);
      //if (obs=="nhf")  hv->SetBinContent(n,-0.10);
      //if (obs=="gammaf")  hv->SetBinContent(n,+0.05);
    }
