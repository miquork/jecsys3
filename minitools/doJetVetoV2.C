...
void doJetVetoV2s(string run) {
  ...
  if (run=="2023BCv123") {
    lumi_136TeV = "Run2023BCv123, 9.3 fb^{-1}"; // 8.728+0.589
    f = new TFile("rootfiles/Iita_20230814/nano_data_out_2023BCv123_v1.root","READ");
  }
  if (run=="2023Cv4") {
    lumi_136TeV = "Run2023Cv4, 4.3 fb^{-1}";
    f = new TFile("rootfiles/Iita_20230814/nano_data_out_2023Cv4_v1.root","READ");
  }
  if (run=="2023BC") {
  ...
  for (int ih = 0; ih != nh; ++ih) {
    ...
    c1nom->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_%s_%s_Run%s.pdf",
    ...
    c1abs->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_%s_%s_Run%s.pdf",
  ...
  if (oneTrig!="" && oneHist!="") {
    c1nom->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_%s_%s_%s_Run%s.pdf",
		       oneTrig.c_str(), oneHist.c_str(),
		       doPull ? "nompull" : "absrel",run.c_str()));
    c1abs->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_%s_%s_%s_Run%s.pdf",
		       oneTrig.c_str(), oneHist.c_str(),
		       doPull ? "abspull" : "absrel",run.c_str()));
  }
  else {
    c1nom->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_h2nommap_Run%s.pdf",run.c_str()));
    c1abs->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_h2absmap_Run%s.pdf",run.c_str()));
  }
  ...
  if (h2jes) {
    ...
    c2->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_x2jes_%s.pdf",run.c_str()));
    ...
    c2norm->SaveAs(Form("pdf/doJetVetoV2/doJetVetoV2_x2jesnorm_%s.pdf",run.c_str()));
