...
void FikriDepths() {
  ...
  TH1D *hu_hf = tdrHist("hu_hf","Jet energy fraction vs tag",0,1.0,
			"Jet #eta",-5.19,5.19);
  TH1D *hd_hf = tdrHist("hd_hf","Data / MC",0.0,2.0,
			"Jet #eta",-5.19,5.19);
  extraText = "Private";
  lumi_136TeV = Form("20%s vs %s",chfd,chfm);
  TCanvas *c3 = tdrDiCanvas("c3",hu_hf,hd_hf,8,11);
