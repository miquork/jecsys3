// Purpose: Modify Zflavor.C for gamma+jet processing
...
double effScaleG = 1; // turn off (no extra scaling)
bool useDataFracs = true;
bool useDataEff = false;//true; // does not work yet
bool useMCEff = true;//false;//true;

// Correct for gluon efficiency
bool fixGluon = true;//false;//true; // default:true
// Override gluon SF with ad-hoc scale
double fixGluonAdhocSF = 0.97;//0;//0.95;  // default:0 (none), 0.95 for Run2Test 1.3
...
bool fixHF = true;//false;//true;
//bool fixB = true;
//bool fixC = true;
// Override charm and/or bottom SF+mistag with ad-hoc scale
double fixCharmAdhocSF = 1.25;//2.3; // default:0 (none)
double fixCharmAdhocMistag = 1.25;//2.3; // default:0 (none)
double fixBottomAdhocSF = 0.95;//0.85; // default:0 (none)
double fixBottomAdhocMistag = 0.95;//0.85; // default:0 (none)

//bool useSmoothMCEff = true;
// Allow to test rough scale changes for response
double gluonScale = 1;//0.95;//0.990;//0.995;//0.970; // default:1
...
//void Gflavor(double EtaMax = 1.3, string iov = "2025CDEFG") {
//void Gflavor(double EtaMax = 1.3, string iov = "2024FLAVOR") {
void Gflavor(double EtaMax = 1.3, string iov = "2025FLAVOR") {

  gROOT->ProcessLine(".! mkdir pdf/Gflavor");
  gROOT->ProcessLine(".! touch pdf/Gflavor");
  ...
  // Region with stable unfolded behavior for q, g, b (? test1)
    {40, 50, 60, 75, 90, 110, 130, 175, 230, 300, 400, 500, 600, 700, 850};
  // Full range for gamma+jet
  // {15, 20, 25, 30, 35, 40, 50, 60, 75, 90, 110, 130, 175, 230, 300, 400, 500, 600, 700, 850, 1000, 1200, 1450, 1750, 2100, 2500, 3000};
  ...
  */
  ...
  cmsText = "HIP CMS";
  extraText = "Private";
  //lumi_13TeV = "[Gamma+jet] Run 3 2025CDEF, 64.5 fb^{-1}";
  lumi_136TeV = Form("[#gamma+jet] %s, %s",ci,mlum[ci].c_str());
  ...
  */
  ...
  gPad->Update();
  c0->SaveAs(Form("pdf/Gflavor/Gflavor_counts_v1_Eta%1.0f_%s.pdf",10*etamax,ci));
  
  //lumi_13TeV = "UL17+18, 101.4 fb^{-1}";
  TH1D *h1 = (TH1D*)href->Clone("h1");

  // Flavor fractions vs tag (smoothing)
  TCanvas *c1 = tdrCanvas("c1b",h1,8,11,kSquare);
  ...
  gPad->Update();
  c1->SaveAs(Form("pdf/Gflavor/Gflavor_mcfrac_v1_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  gPad->Update();
  c2->SaveAs(Form("pdf/Gflavor/Gflavor_mcresp_v1_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  for (int ipt = 0; ipt != npt; ++ipt) {
	...
	//double effsfh = (isc ? 2.3 : 0.85); // PATCH
	if (fixCharmAdhocSF!=0 && isc) effsfh = fixCharmAdhocSF;
	if (fixBottomAdhocSF!=0 && isb) effsfh = fixBottomAdhocSF;
	...
	double effsfq = (jh==jc ? 
			 ...
			 quarkEffSF(ptbins[ipt],ptbins[ipt+1],ineffqsf));
	//if (fixCharmAdhocMistag!=0 && isc) ineffqsf = fixCharmAdhocMistag;
	//if (fixBottomAdhocMistag!=0 && isb) ineffqsf = fixBottomAdhocMistag;
	if (fixCharmAdhocMistag!=0 && isc) effsfq = fixCharmAdhocMistag;
	if (fixBottomAdhocMistag!=0 && isb) effsfq = fixBottomAdhocMistag;
  ...
  c0->SaveAs(Form("pdf/Gflavor/Gflavor_counts_v2_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  c1->SaveAs(Form("pdf/Gflavor/Gflavor_mcfrac_v2_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  c0->SaveAs(Form("pdf/Gflavor/Gflavor_counts_v3_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  gPad->Update();
  c3->SaveAs(Form("pdf/Gflavor/Gflavor_mcfrac_v3_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  c2->SaveAs(Form("pdf/Gflavor/Gflavor_mcresp_v2_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  gPad->Update();
  c4->SaveAs(Form("pdf/Gflavor/Gflavor_resp_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  gPad->Update();
  c5->SaveAs(Form("pdf/Gflavor/Gflavor_fjes_Eta%1.0f_%s.pdf",10*etamax,ci));
  ...
  //TH1D *hsp = (TH1D*)dr->Get("herr_pjt"); assert(hsp);
  ...
  // Ratio to inclusive HDM for data, MC, fits
  TCanvas *c6 = tdrCanvas("c5",h6,8,11,kSquare);
  ...
  tdrDraw(hflp, "E2", kNone,kRed+1, kSolid,-1,1001,kRed-9);
  ...
  leg6->AddEntry(hflp,"FlavorGJet syst.","F");
  ...
  gPad->RedrawAxis();
  gPad->Update();
  c6->SaveAs(Form("pdf/Gflavor/Gflavor_datamc_Eta%1.0f_%s.pdf",10*etamax,ci));

  // Save results for comparing to Z+jet
  TFile *ferc = new TFile("pdf/Gflavor/GflavorRun3Test.root","RECREATE");
...
double gluonEffSF(double ptmin, double ptmax, double &ineffsf, double mceff) {
  ...
  double effm = (mceff!=0 ? mceff : 
		 (effScaleG==-1 ? 1./f1w->Eval(pt) : effScaleG)*f1mg->Eval(pt));
...
double quarkEffSF(double ptmin, double ptmax, double &ineffsf) {
  ...
  double effm = (effScaleG==-1 ? 1./f1w->Eval(pt) : effScaleG)*f1mq->Eval(pt);
