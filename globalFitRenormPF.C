...
void globalFitRenormPF(string s = "2025CDEFGH") {
...
void globalFitPrecombinePF(string s) {
  ...
  // incjet: 18, 21, 24, 28, 32, 37, 43, 49, 56, 64, 74, 84, 97, 114, 133, 153, 174, 196, 220, 245, 272, 300, 330, 362, 395, 430, 468, 507, 548, 592, 638, 686, 737, 790, 846, 905, 967, 1032, 1101, 1172, 1248, 1327, 1410, 1497, 1588, 1684, 1784, 1890,
  const double x[] =
    {15, 20, 25, 30, 35, 40, 45, 50, 60, // Z/W only
     70, 85, 105, 130, // +gamjet
     175, 230, 300, 400, 500, // still Z/gamma domain
     //592, 686, 790, 905, 1032, 1172, 1327, 1497, // multijet
     600, 800, 1000, 1200, 1450, // gamjet+multijet x2
     //1684, 1890, 2116, 2366, 2640, 2941, 3273, 5220}; // multijet
     1890, 2366, 2941, 5220}; // multijet x2
...
void globalFitPrecombineHDM(string s, bool addMJ, bool scaleRes) {
  ...
  sets.push_back("zjet");
  //sets.push_back("hadw");
  //if (s!="2025CDEFGH") sets.push_back("incjet");
  ...
  for (unsigned int id = 0; id != dirs.size(); ++id) {
	...
	if (f1->GetParError(0)>0) { // check that something was fit
	  ...
	  // Special treatment for multijet to reference it to lower pT JES,
	  // where JES is from the sum of all previous samples
	  if (ss=="multijet") {
	    TGraphErrors *gc =
	      (TGraphErrors*)f->Get(Form("%s/eta00-13/crecoil_%s_a100",cd,cs));
	    assert(gc);
	  ...
	  // Add sample statistically to all previous ones
	  hs->SetBinContent(i, (yw0 + yw) / (w0 + w));
	  hs->SetBinError(i, (eyw0 + eyw) / (w0 + w));
	  hs2->SetBinContent(i, (y2w0 + y2w) / (w0 + w));
	  hsw->SetBinContent(i, w0 + w);
